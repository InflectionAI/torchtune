...
class Qwen2_5Tokenizer(Qwen2Tokenizer):  # noqa: N801
    ...
    def __init__(
        ...
        *,
        ...
        # Media tokens
        #print(f"_tokenizer.py self.special_tokens: {self.special_tokens}")
        self.image_id = self.special_tokens["<|image_pad|>"]
        self.video_id = self.special_tokens["<|video_pad|>"]
        ...
        *,
        ...
        for i, message in enumerate(templated_messages):
            ...
            for item in message.content:
                print(f"_tokenizer.py item: {item}")
                if item["type"] == "text":
                    tokens.extend(
                        self.encode(
                            item["content"],
