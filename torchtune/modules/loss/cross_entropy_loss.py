...
class MedusaCrossEntropyLoss(nn.Module, SFTLoss):
    ...
    def forward(
        ...
        for i in range(self.medusa_num_heads):
            ...
            hidden_chunks = hidden_state.tensor_split(self.num_output_chunks, dim=1)
            target_chunks = medusa_targets.tensor_split(self.num_output_chunks, dim=1)

            # Compute cross-entropy loss for the chunks
            loss_per_head = 0.0
            for idx in range(len(hidden_chunks)):
                loss_per_head += self.compute_cross_entropy(hidden_chunks[idx],target_chunks[idx], head_index = i)
...
class LinearCrossEntropyLoss(nn.Module, SFTLoss):
    ...
    def forward(
        ...
        total_loss = 0.0
        for idx in range(len(hidden_chunks)):
            total_loss += self.compute_cross_entropy(hidden_chunks[idx],target_chunks[idx],)
