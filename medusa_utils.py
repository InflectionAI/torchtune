...
def generate_medusa_buffers(medusa_choices, device="cuda"):
    ...
    for i in range(len(depth_counts)):
        for j in range(depth_counts[i]):
            ...
            if len(cur_medusa_choice) == 1:
                continue
            ancestor_idx = []
            # 1 is being added to the col and row number in most places because the first row and column is the base_model_pred
            for c in range(len(cur_medusa_choice) - 1):
                ancestor_idx.append(sorted_medusa_choices.index(cur_medusa_choice[:c+1]) + 1)
            medusa_attn_mask[j + start + 1, ancestor_idx] = 1 # 1 added in the row indexing is because j is 0 for the first element in the new depth.
        start += depth_counts[i] 
    
    # branching factor is topk
    ...
    max_length = max([len(x) for x in retrieve_indices_nest]) # longest path in tree
    retrieve_indices = [pad_path(path, max_length) for path in retrieve_indices_nest] # pad all paths to be of longest path length
...
def create_medusa_causal_mask(
    ...
    Args:
        ...
        device: Device to create tensor on
        dtype: Data type for the mask
        medusa_mask: Medusa attention mask of shape [current_seq_len, current_seq_len]
    
    Returns:
        Causal mask of shape [batch_size, current_seq_len, max_cache_size]
    """
    combined_attention_mask = create_causal_mask(batch_size, current_seq_len, cached_seq_len, max_cache_size, device, dtype)
    
    # Apply Medusa mask to the new tokens region only
    # Target: [batch, :, cached_seq_len:cached_seq_len+current_seq_len, cached_seq_len:cached_seq_len+current_seq_len]
    # This corresponds to the bottom-right current_seq_len × current_seq_len block
    
    # Convert to float for min() operation
    combined_attention_mask = combined_attention_mask.float()
    
    # Apply medusa_mask to the new tokens region
    # Shape: [batch, current_seq_len, current_seq_len]
    new_tokens_region = combined_attention_mask[:, :, cached_seq_len:cached_seq_len+current_seq_len]
    
    # Apply the medusa_mask: where medusa_mask == 0, set to minimum value
    new_tokens_region[medusa_mask == 0] = combined_attention_mask.min()
    
    # Update the combined mask
    combined_attention_mask[:, :, cached_seq_len:cached_seq_len+current_seq_len] = new_tokens_region
...
def get_attn_layer(model):
    ...
    return layer, curr_kv_len

def initialize_medusa(input_ids, model):
    """
    ...
    # Getting max_kv_cache_size from the attn layer since the model fwd pass needs this
    layer, curr_kv_len = get_attn_layer(model)
    assert(curr_kv_len == 0)
    k_cache_shape = layer.attn.kv_cache.k_cache.shape
    (bs, num_kv_heads, max_cache_size, head_dim) = k_cache_shape

    # Getting other arguments needed by the model fwd pass
    ...
    with sdp_math_context():
        output = model(input_ids, mask=causal_mask, input_pos=input_pos)
    base_logits = output[0]
    vocab_dim = base_logits.shape[-1]
    medusa_logits = output[1:]
    medusa_logits = torch.stack(medusa_logits, dim=0) # shape: (n_heads, bs, seq_len, vocab_dim)
    

    return medusa_logits, base_logits 
...
def get_mask_input_pos(model, input_ids):
    # Getting max_kv_cache_size from the attn layer since the model fwd pass needs this
    layer, curr_kv_len = get_attn_layer(model)
    k_cache_shape = layer.attn.kv_cache.k_cache.shape
    (bs, num_kv_heads, max_cache_size, head_dim) = k_cache_shape
...
def get_medusa_mask(model, input_ids):
    # Getting max_kv_cache_size from the attn layer since the model fwd pass needs this
    layer, curr_kv_len = get_attn_layer(model)
    k_cache_shape = layer.attn.kv_cache.k_cache.shape
    (bs, num_kv_heads, max_cache_size, head_dim) = k_cache_shape
    ...
    causal_mask = create_medusa_causal_mask(bs, curr_seq_len, curr_kv_len, max_cache_size, device, model_dtype)
    return causal_mask
...
def tree_decoding(
    ...
    '''
    # what is my attn_mask here and what is my position_ids
    ...
    tree_medusa_logits = torch.stack(medusa_logits, dim=0) # shape: (n_heads, bs, seq_len, vocab_dim)
...
def update_inference_inputs(
    ...
    # kv cache handling for torchtune model
    valid_kv_len += accept_length
        
    # Reset cache to valid length (remove invalid medusa predictions)
    print("-----------------------------------------------------")
    layer, invalid_kv_len = get_attn_layer(model)
    print('cache len before (invalid):', invalid_kv_len)
    model.revert_cache_to_valid_length(valid_kv_len)
    layer, valid_kv_len = get_attn_layer(model)
    print('cache len now (valid):', valid_kv_len)
